...
if ($result->num_rows > 0) {
    ...
    $query = "SELECT * FROM subscriptions WHERE id_user='$id'";
    $result = mysqli_query($conn, $query);
...
if (isset($_POST['submit-identity'])) {
    ...
    if ($new_email == $email) {
        $query = "UPDATE users SET full_name='$new_name' WHERE id_user='$id'";
        $result = mysqli_query($conn, $query);
...
if (isset($_POST['submit-password'])) {
    $curr_password = hash("sha256", $_POST['input-password']);
    $new_password = hash("sha256", $_POST['input-new-password']);
    $new_conf_password = hash("sha256", $_POST['input-conf-new-password']);
    ...
    if ($new_password == $new_conf_password) {
        if ($curr_password == $password) {
            ...
            if ($result) {
                ...
                header("Location: profile.php");
                $success_reset = "Password reset completed! ";
            }
        } else {
            $error_curr_password = "Your password is wrong.";
        }
    } else {
        $error_conf_password = "Confirmation password does not match.";
    }
}
            ...
            <div class="card" style="margin-bottom: 20px;">
                ...
                <div class="card-body" style="text-align: start; padding: 40px;">
                    <form method="POST">
                        ...
                        <div class="form-group row mx-auto" style="width: fit-content;">
                            <button name="submit-password" type="submit" class="btn btn-secondary">Reset</button>
                        </div>
                        <?php
                        if (isset($error_curr_password)) echo "<div class='alert alert-danger'>" . $error_curr_password . "</div>";
                        if (isset($error_conf_password)) echo "<div class='alert alert-danger'>" . $error_conf_password . "</div>";
                        if (isset($success_reset)) echo "<div class='alert alert-success'>" . $success_reset . "</div>";
                        ?>
                    </form>
                </div>
            </div>
                ...
                <div class="card-body" style="text-align: start; padding: 40px;">
                    <form method="POST">
                        <div class="form-group row" style="margin-bottom: 20px;">
                            ...
                            <div class="col-sm-9">
                                <input type="text" class="form-control" id="input-status" name="input-status" value="<?= $status; ?>" readonly>
                            </div>
                        </div>
                        <div class="form-group row" style="margin-bottom: 20px;">
                            <label for="input-subs-date" class="col-sm-3 col-form-label">Subscription Date</label>
                            <div class="col-sm-9">
                                <input type="date" class="form-control" id="input-subs-date" name="input-subs-date" value="<?php if (isset($date)) echo $date ?>" readonly>
                            </div>
                        </div>
                        <div class="form-group row mx-auto" style="width: fit-content;">
